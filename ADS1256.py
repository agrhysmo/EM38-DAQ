...
#ScanMode 0= 8 single channels- 1=4 differential channels- 2= 0,1,2,3 singol 
                                                        #ended 4-5,6-7 vdiff
ScanMode = 2 
...
# command definition
CMD = {'CMD_WAKEUP' : 0x00,     # Completes SYNC and Exits Standby Mode (00h)
       'CMD_RDATA' : 0x01,      # Read Data (01h)
       'CMD_RDATAC' : 0x03,     # Read Data Continuously (03h)
       'CMD_SDATAC' : 0x0F,     # Stop Read Data Continuously (0Fh)
       'CMD_RREG' : 0x10,       # Read from REG rrr 0001 rrrr (1xh)
       'CMD_WREG' : 0x50,       # Write to REG rrr 0101 rrrr (5xh)
       'CMD_SELFCAL' : 0xF0,    # Offset and Gain Self-Calibration (F0h)
       'CMD_SELFOCAL' : 0xF1,   # Offset Self-Calibration (F1h)
       'CMD_SELFGCAL' : 0xF2,   # Gain Self-Calibration (F2h)
       'CMD_SYSOCAL' : 0xF3,    # System Offset Calibration (F3h)
       'CMD_SYSGCAL' : 0xF4,    # System Gain Calibration (F4h)
       'CMD_SYNC' : 0xFC,       # Synchronize the A/D Conversion (FCh)
       'CMD_STANDBY' : 0xFD,    # Begin Standby Mode (FDh)
       'CMD_RESET' : 0xFE,      # Reset to Power-Up Values (FEh)
      }
...
class ADS1256:
        ...
        if (ScanMode == 1):
            if Channal == 0:
                self.ADS1256_WriteReg(REG_E['REG_MUX'], (0 << 4) | 1)   
                                                #DiffChannal  AIN0-AIN1
            elif Channal == 1:
                self.ADS1256_WriteReg(REG_E['REG_MUX'], (2 << 4) | 3)   
                                                #DiffChannal   AIN2-AIN3
            elif Channal == 2:
                self.ADS1256_WriteReg(REG_E['REG_MUX'], (4 << 4) | 5)   
                                                #DiffChannal    AIN4-AIN5
            elif Channal == 3:
                self.ADS1256_WriteReg(REG_E['REG_MUX'], (6 << 4) | 7)   
                                                #DiffChannal   AIN6-AIN7
        return 0

    def ADS1256_SetMultyChannal(self, Channal):
        if (ScanMode == 2):
            if (Channal <= 3):
                self.ADS1256_WriteReg(REG_E['REG_MUX'], (Channal<<4) | (1<<3))   
                                #Single ended Channel AIN0, AIN1, AIN2, AIN3
            elif (Channal == 4):
                self.ADS1256_WriteReg(REG_E['REG_MUX'], (4 << 4) | 5)   
                                #DiffChannal  AIN4-AIN5
            elif (Channal == 5):
    ...
    def ADS1256_init(self):
        ...
        if id == 3 :
            ...
            return -1
        self.ADS1256_ConfigADC(ADS1256_GAIN_E['ADS1256_GAIN_1'], 
    ...
    def ADS1256_Read_ADC_Data(self):
        ...
        if(read > 0x7fffff):
            return (read-0x100000000)
        else:
            return read
 
    def ADS1256_GetChannalValue(self, Channel):
        if(ScanMode == 0):  #0 Single-ended input 8 channel
            ...
            if(Channel >= 6):
                ...
                self.ADS1256_SetMultyChannal(Channel)
        else:
            return 0
